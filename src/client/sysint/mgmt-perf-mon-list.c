 ...
 */
PVFS_error PVFS_imgmt_perf_mon_list(PVFS_fs_id fs_id,
                                    ...
                                    void *user_ptr)
{
    ...
    if ((server_count < 1) || (history_count < 1) || !key_count
             || (*key_count < 1) || !perf_matrix || !addr_array
             || !end_time_ms_array)
    {
	return -PVFS_EINVAL;
    }
    ...
    /* perf_matrix should be server_count * key_count * history count */
    sm_p->u.perf_mon_list.perf_matrix = perf_matrix;
    sm_p->u.perf_mon_list.server_count = server_count;
    sm_p->u.perf_mon_list.key_count = key_count;
    sm_p->u.perf_mon_list.req_keys = *key_count;
 ...
 */
PVFS_error PVFS_mgmt_perf_mon_list(PVFS_fs_id fs_id,
                                   const PVFS_credential *credential,
                                   int64_t **perf_matrix,
                                   uint64_t *end_time_ms_array,
                                   PVFS_BMI_addr_t *addr_array,
                                   ...
                                   PVFS_hint hints)
{
    ...
    ret = PVFS_imgmt_perf_mon_list(fs_id,
                                   credential,
                                   perf_matrix,
                                   end_time_ms_array,
                                   addr_array,
...
static PINT_sm_action mgmt_perf_mon_list_setup_msgpair(struct PINT_smcb *smcb,
                                                       job_status_s *js_p)
{
	...
	PINT_SERVREQ_MGMT_PERF_MON_FILL(msg_p->req,
                                        ...
                                        *sm_p->u.perf_mon_list.key_count,
...
static PINT_sm_action mgmt_perf_mon_list_cleanup(struct PINT_smcb *smcb,
                                                 job_status_s *js_p)
{
    struct PINT_client_sm *sm_p = PINT_sm_frame(smcb, PINT_FRAME_CURRENT);
    int i = 0, errct = 0;
				 ...
				 int i)
{
	...
	*sm_p->u.perf_mon_list.key_count = resp_p->u.mgmt_perf_mon.key_count;

        for (h = 0; h < sm_p->u.perf_mon_list.history_count; h++)
        {
            int coff = h * (sm_p->u.perf_mon_list.req_keys + 2);
            int soff = h * (resp_p->u.mgmt_perf_mon.key_count + 2);
	    memcpy(&sm_p->u.perf_mon_list.perf_matrix[i][coff],
	           &resp_p->u.mgmt_perf_mon.perf_array[soff],
	           (*sm_p->u.perf_mon_list.key_count + 2) * sizeof(int64_t));
        }
    }
