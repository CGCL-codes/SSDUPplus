...
#ifndef WIN32
# 137 "src/client/sysint/sys-create.sm"
#endif
    ...
    void *user_ptr)
{
    ...
    {
        gossip_debug(GOSSIP_CLIENT_DEBUG, "Replacing group gid %d with %d "
                                  ...
                                  int index)
{
    ...
    /* otherwise, store the data returned from the server */
    sm_p->u.create.server_resp.metafile_handle = resp_p->u.create.metafile_handle;
    sm_p->u.create.server_resp.stuffed         = resp_p->u.create.stuffed;

    ret = PINT_copy_object_attr(&(sm_p->u.create.server_resp.metafile_attrs),
                                &(resp_p->u.create.metafile_attrs));
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    gossip_debug(GOSSIP_CLIENT_DEBUG, "create: encrypt dirent %s into hash value %llu.\n",                         
            sm_p->u.create.object_name,
            llu(dirdata_hash));

    gossip_debug(GOSSIP_CLIENT_DEBUG,
            "create: dist_dir_bitmap:\n");
    for(i = sm_p->getattr.attr.dist_dir_attr.bitmap_size - 1; i >= 0 ; i--)
    {
        c = (unsigned char *)(sm_p->getattr.attr.dist_dir_bitmap + i);
        gossip_debug(GOSSIP_CLIENT_DEBUG," i=%d : %02x %02x %02x %02x\n"
                                        , i, c[3], c[2], c[1], c[0]);
    }
    gossip_debug(GOSSIP_CLIENT_DEBUG, "\n");
    ...
    gossip_debug(GOSSIP_CLIENT_DEBUG, "create: selecting bucket No.%d from dist_dir_bitmap.\n", 
            dirdata_server_index);
        
    gossip_debug(GOSSIP_CLIENT_DEBUG,
                 "create: %s: posting crdirent req: parent handle: %llu, "
                 "name: %s, handle: %llu, dirdata_handle: %llu\n",
                 __func__,
                 llu(sm_p->object_ref.handle), sm_p->u.create.object_name,
                 llu(sm_p->u.create.server_resp.metafile_handle),
    ...
    PINT_SERVREQ_CRDIRENT_FILL(
        ...
        *sm_p->cred_p,
        sm_p->u.create.object_name,
        sm_p->u.create.server_resp.metafile_handle,
        sm_p->object_ref.handle,
        sm_p->getattr.attr.dirdata_handles[dirdata_server_index],
        sm_p->object_ref.fs_id,
        sm_p->hints);

    gossip_debug(GOSSIP_CLIENT_DEBUG,"create: msg_p->req.op(%d)\n",msg_p->req.op);
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
       ...
       sm_p->u.create.retry_count < sm_p->msgarray_op.params.retry_limit)
    {
        ...
        gossip_debug(GOSSIP_CLIENT_DEBUG, "create: received -PVFS_EAGAIN, will retry getattr and crdirent (attempt number %d).\n",
                     sm_p->u.create.retry_count);
        gossip_debug(GOSSIP_CLIENT_DEBUG,"%s:sm_p->getattr.attr.mask(0x%0x)\n",__func__,sm_p->getattr.attr.mask);
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    {
        metafile_ref.handle = sm_p->u.create.server_resp.metafile_handle;
        ...
        if(sm_p->u.create.dist)
        {
            sm_p->u.create.server_resp.metafile_attrs.u.meta.dist = sm_p->u.create.dist;
            sm_p->u.create.server_resp.metafile_attrs.u.meta.dist_size = PINT_DIST_PACK_SIZE(sm_p->u.create.dist);
            sm_p->u.create.server_resp.metafile_attrs.mask |= PVFS_ATTR_META_DIST;
        }


        /* Add PVFS_ATTR_DATA_SIZE to attribute mask; we know that the file size is zero, since we just created it. */
        sm_p->u.create.server_resp.metafile_attrs.mask |= PVFS_ATTR_DATA_SIZE;
        ...
        ret = PINT_acache_update(metafile_ref,
                                 &sm_p->u.create.server_resp.metafile_attrs,
                                 &data_size);
        if(ret < 0)
        {
            js_p->error_code = ret;
    ...
    if(sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_array)
    {
        free(sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_array);
        sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_array = NULL;
    }
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    gossip_debug(GOSSIP_CLIENT_DEBUG, "parent owner: %d, group: %d, perms: %o\n",
        (int)attr->owner, (int)attr->group, (int)attr->perms);

    PINT_debug_capability(&sm_p->getattr.attr.capability,"create_parent_getattr_inspect");
    ...
    PVFS_hint_add(&sm_p->hints, PVFS_HINT_OWNER_GID_NAME, sizeof(uint32_t), 
                  &sm_p->u.create.attr.group);
#endif
    
    /* !!! change more later */
    gossip_debug(GOSSIP_CLIENT_DEBUG, "create_parent_getattr: [%p] "
        "dirdata_server_count = %d "
        "dfile_count     = %d "
        "dist_name_len   = %d "
        "dist_params_len = %d\n",
        attr,
    ...
    /* !!! change more later */
    for (tmp_i = 0; tmp_i < attr->dist_dir_attr.num_servers; tmp_i++)
    {
        gossip_debug(GOSSIP_CLIENT_DEBUG,
                     "                           dirdata_handle[%d] = %llu\n",
                     tmp_i, llu(attr->dirdata_handles[tmp_i]));
    }

    /* may just use sm_p->getattr.attr.u.dir...
    sm_p->u.create.dirent_handle = malloc(
        sizeof(*attr->dirdata_handles) * attr->dist_dir_attr.num_servers);
    if (!sm_p->u.create.dirent_handle)
    {
        return -PVFS_ENOMEM;
    }
    memcpy(sm_p->u.create.dirent_handle,
           ...
           sizeof(*attr->dirdata_handles) * attr->dist_dir_attr.num_servers);
    sm_p->u.create.dirent_file_count = attr->dist_dir_attr.num_servers;
    */
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    ret = PINT_msgpairarray_init(&sm_p->msgarray_op, (sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_count+1));
    if(ret != 0)
    {
        gossip_err("Failed to initialize %d msgpairs\n", (sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_count+1));
        js_p->error_code = ret;
        return(SM_ACTION_COMPLETE);
    }
    ...
    */
    foreach_msgpair(&sm_p->msgarray_op, msg_p, i)
    {
        ...
        if (i == sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_count)
        {
            ...
            msg_p->fs_id = sm_p->object_ref.fs_id;
            msg_p->handle = sm_p->u.create.server_resp.metafile_handle;
            msg_p->retry_flag = PVFS_MSGPAIR_NO_RETRY;
        ...
        else
        {
            PINT_SERVREQ_REMOVE_FILL(
                ...
                *sm_p->cred_p,
                sm_p->object_ref.fs_id,
                sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_array[i],
                sm_p->hints);

            msg_p->fs_id = sm_p->object_ref.fs_id;
            msg_p->handle = sm_p->u.create.server_resp.metafile_attrs.u.meta.dfile_array[i];
            msg_p->retry_flag = PVFS_MSGPAIR_NO_RETRY;
            msg_p->comp_fn = create_delete_handles_comp_fn;
