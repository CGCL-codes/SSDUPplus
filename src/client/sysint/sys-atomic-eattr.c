...
static int atomic_eattr_comp_fn(
    void *v_p,
...
static PINT_sm_action atomic_eattr_setup_msgpair(
...
static PINT_sm_action atomic_eattr_cleanup(
	struct PINT_smcb *smcb, job_status_s *js_p);
...
struct PINT_state_machine_s pvfs2_client_atomic_eattr_sm = {
	.name = "pvfs2_client_atomic_eattr_sm",
	.first_state = &ST_getattr
};

static struct PINT_state_s ST_getattr = {
	 .state_name = "getattr" ,
	 .parent_machine = &pvfs2_client_atomic_eattr_sm ,
	 .flag = SM_JUMP ,
...
static struct PINT_state_s ST_setup_msgpair = {
	 ...
	 .parent_machine = &pvfs2_client_atomic_eattr_sm ,
	 .flag = SM_RUN ,
	 .action.func = atomic_eattr_setup_msgpair ,
	 .pjtbl = NULL ,
...
static struct PINT_state_s ST_cleanup = {
	 ...
	 .parent_machine = &pvfs2_client_atomic_eattr_sm ,
	 .flag = SM_RUN ,
	 .action.func = atomic_eattr_cleanup ,
	 .pjtbl = NULL ,
...
#ifndef WIN32
# 66 "src/client/sysint/sys-atomic-eattr.sm"
#endif
        ...
        void *user_ptr)
{
    ...
    if(gossip_debug_enabled(GOSSIP_SETEATTR_DEBUG))
    {
        ...
        for(i = 0; i < nkey; ++i)
        {
            char *valBuf = (char *)val_array[i].buffer;
            int j;
    ...
    PINT_smcb_alloc(&smcb, PVFS_SYS_ATOMICEATTR,
    ...
    sm_p->u.atomiceattr.val_array = val_array;
        ...
        PVFS_hint hints)
{
    ...
    ret = PVFS_isys_atomiceattr_list(ref, credential, nkey, key_array,
                    val_array, resp_p, flags, &op_id, opcode, hints, NULL);
    if (ret)
    {
        ...
        PVFS_hint hints)
{
    PVFS_sysresp_geteattr resp_p;
    PVFS_error tmp_err;
    resp_p.val_array = response;
    resp_p.err_array = &tmp_err;
    return PVFS_sys_atomiceattr_list(ref, credential, 1, key_p, val_p,
                                  &resp_p, flags, opcode, hints);
}
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    /* this will be the array of value buffer sizes for the old attributes */
    sm_p->u.atomiceattr.size_array =
             (PVFS_size *)malloc(sm_p->u.atomiceattr.nkey *
                                 sizeof (PVFS_size));

    /* fill the array pulling out of the value array */
    /* we don't want to send the unused fields in this array */
    for (i = 0; i < sm_p->u.atomiceattr.nkey; i++)
        sm_p->u.atomiceattr.size_array[i] =
            sm_p->u.atomiceattr.resp_p->val_array[i].buffer_sz;
   
    PINT_msgpair_init(&sm_p->msgarray_op);
    msg_p = &sm_p->msgarray_op.msgpair;

    PINT_SERVREQ_ATOMICEATTR_FILL(
            msg_p->req,
        ...
        struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    free(sm_p->u.atomiceattr.size_array);
        ...
        (sm_p->msgarray_op.msgarray[i].op_status == -PVFS_ERANGE))
    {
        ...
        for (k = 0; k < sm_p->u.atomiceattr.nkey; k++)
        {
            gossip_debug(GOSSIP_GETEATTR_DEBUG,"resp_read_sz = %d\n",
                    resp_p->u.atomiceattr.val[k].read_sz);
            gossip_debug(GOSSIP_GETEATTR_DEBUG,"resp_buff_sz = %d\n",
                    resp_p->u.atomiceattr.val[k].buffer_sz);
            ...
            gossip_debug(GOSSIP_GETEATTR_DEBUG,"resp_buff_ = %s\n",
                    (char *)resp_p->u.atomiceattr.val[k].buffer);
