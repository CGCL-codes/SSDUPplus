...
static int wb_parse_config(WB_info*);
...
int main(int argc, char **argv)
{
	...
	ret = wb_parse_config(&my_wb_info);
	if (ret)
    {
        gossip_err("Error: Please check write back config file.\n");
        gossip_err("Error: Server aborting.\n");
        ret = -PVFS_EINVAL;
        goto server_shutdown;
    }
...
static int wb_parse_config(WB_info *wb_info)
{
	FILE *wb_config;
	
	char buffer[1024];
	char buffer_no_space[1024];
	int i=0,j=0;
	char *p;
	
	wb_config = fopen(wb_conf,"r");

	gossip_set_debug_mask(1, GOSSIP_SERVER_DEBUG);
		
	while(fgets(buffer,1024,wb_config)!=NULL)
	{
		i=0;
		j=0;
		while(buffer[i]!='\0')
		{
			...
			{
				buffer_no_space[j] = buffer[i];
				i++;
				j++;
			}
		}
		buffer_no_space[j] = '\0';
		p = strtok(buffer_no_space,":");
		if(strcmp(p,"buff_path_1")==0)
		{
			p = strtok(NULL,":");
			sscanf(p,"%s",wb_info->buff_path_1);
		}
        else if(strcmp(p,"buff_path_2")==0)
        {
            p = strtok(NULL,":");
            sscanf(p,"%s",wb_info->buff_path_2);
        }
		else if(strcmp(p,"buff_threshold")==0)
		{
            //kb
			p = strtok(NULL,":");
            char ss[16];
            ...
            sscanf(p,"%[0-9]",ss);
            sscanf(ss,"%lld",&tmp);
            if( (strstr(p,"MB") || strstr(p,"mb")||strstr(p,"M")|| strstr(p,"m")) !=NULL)
            {
                wb_info->buff_threshold = tmp*1024;
            }
            else if( (strstr(p,"GB") || strstr(p,"gb")||strstr(p,"G")|| strstr(p,"g")) !=NULL)
            {
                wb_info->buff_threshold = tmp*1024*1024;
            }
            else if( (strstr(p,"TB") || strstr(p,"tb")||strstr(p,"T")|| strstr(p,"t")) !=NULL )
            {
                wb_info->buff_threshold = tmp*1024*1024*1024;
            }
            else
            {
                wb_info->buff_threshold = tmp;
            }
		}
		
        else if(strcmp(p,"high_watermark")==0)
        {
            p = strtok(NULL,":");
            sscanf(p,"%f",&(wb_info->high_watermark));
        }
        else if(strcmp(p,"low_watermark")==0)
        {
            p = strtok(NULL,":");
            sscanf(p,"%f",&(wb_info->low_watermark));
        }
		else if(strcmp(p,"rs_length")==0)
        {
            p = strtok(NULL,":");
    ...
    gossip_set_debug_mask(0, GOSSIP_SERVER_DEBUG);
	fclose(wb_config);
	return 0;
}
...
static int server_parse_cmd_line_args(int argc, char **argv)
{
    ...
    if (argv[optind][0] != '/')
    {
        ...
        if( strncpy(wb_conf, startup_cwd, PATH_MAX) == NULL )
        {
            gossip_err("Failure copying created full path into configuration "
                       "file path\n");
            goto parse_cmd_line_args_failure;
    ...
    else
    {
        ...
        if( strncpy( wb_conf, argv[optind++], PATH_MAX) == NULL )
        {
            gossip_err("Failure copying configuration file path\n");
            goto parse_cmd_line_args_failure;
        }
