...
static int check_stuffed(struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    */
    ...
    {    
        ...
        s_op->resp.u.create.metafile_attrs.u.meta.dfile_array = malloc(sizeof(PVFS_handle));
    ...
    struct PINT_smcb *smcb, job_status_s *js_p)
{
    ...
    s_op->val_a[0].buffer_sz =
                    s_op->resp.u.create.metafile_attrs.u.meta.dfile_count * sizeof(PVFS_handle);
...
{    
    ...
    /* NOTE:  Distribution was sent from the client in the original request structure, so there's no need to
     *        to send the same information back.
     */ 
    ...
    if ( s_op->attr.mtime )
    {
       /* Must "un-version" mtime before sending back to client:  When the mtime is stored in dspace in 
        * in the setobj_attribs state, the mtime is "versioned", i.e., the time, given in seconds since
        * the Epoch, is modified using PINT_util_mktime_version().
        */
       s_op->resp.u.create.metafile_attrs.mtime = PINT_util_mkversion_time(s_op->attr.mtime);
    ...
    if ( s_op->resp.u.create.metafile_attrs.u.meta.dfile_count && s_op->resp.u.create.metafile_attrs.u.meta.dfile_array )
    {
